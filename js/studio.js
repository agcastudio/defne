...
- Render ALL doors in the OPEN position with a CLEAR GAP at the doorway: at every door location the wall must show a visible opening (gap) at the drawn door width — the passage itself stays completely EMPTY, never filled by wall, never blocked by a closed leaf. Each door leaf stands open (swung about 90 degrees, following the swing direction drawn in the source plan) and rests against the wall beside the opening, so every doorway reads as an open passage and room connections are clearly visible. Do not render any door closed, and do not remove the door leaves — they stay visible, just open beside their gap.
